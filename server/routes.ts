  ...
  setInterval(async () => {
    try {
      ...
      for (const task of allTasks) {
        ...
        if (diffMs <= 0) {
          ...
          if (task.userTelegramId) {
            await sendTelegramNotification(task.userTelegramId, 
              `⌛ <b>Время вышло!</b>\n\nСрок по задаче "<b>${task.title}</b>" истек. Залог удержан.`);
          }
          continue;
        }
        ...
        if (diffMinutes === 60) {
          await sendTelegramNotification(task.userTelegramId, 
            `🚨 <b>Остался всего 1 час!</b>\nПоднажми, чтобы успеть сдать отчет по задаче "${task.title}".`);
        }

        if (diffMinutes === 1440) {
          await sendTelegramNotification(task.userTelegramId, 
            `⚠️ <b>Остались сутки!</b>\nНе забудь подготовить отчет по задаче "${task.title}".`);
        }
      }
    } catch (err) {
